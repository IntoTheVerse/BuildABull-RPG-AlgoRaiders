...
public class ChestSpawner : MonoBehaviour
{
    ...
    [SerializeField] private List<RangeByLevel> tokenSpawnByLevelList;
    ...
    private void SpawnChest()
    {
        ...
        if (chestSpawnEvent == ChestSpawnEvent.onRoomEntry)
        {
            ...
            chest.Initialize(false, GetHealthPercentToSpawn(healthNum), GetWeaponDetailsToSpawn(weaponNum), GetAmmoPercentToSpawn(ammoNum), GetTokenPercentToSpawn());
        }
        else
        {
            // use materialize effect
            chest.Initialize(true, GetHealthPercentToSpawn(healthNum), GetWeaponDetailsToSpawn(weaponNum), GetAmmoPercentToSpawn(ammoNum), GetTokenPercentToSpawn());
        }
    }
    ...
    /// <summary>
    /// Get token percent to spawn
    /// </summary>
    private bool GetTokenPercentToSpawn()
    {
        // Get ammo spawn percent range for level
        foreach (RangeByLevel spawnPercentByLevel in tokenSpawnByLevelList)
        {
            if (spawnPercentByLevel.dungeonLevel == GameManager.Instance.GetCurrentDungeonLevel())
            {
                return Random.Range(spawnPercentByLevel.min, spawnPercentByLevel.max) > 80;
            }
        }
    ...
    private WeaponDetailsSO GetWeaponDetailsToSpawn(int weaponNumber)
    {
        ...
        WeaponDetailsSO weaponDetails = weaponRandom.GetItem(true);
